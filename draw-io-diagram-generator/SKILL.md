...
### Step 1.5 — Build a Semantic Model (Required for architecture diagrams)

Do **not** jump directly from prose to mxGraph XML. Before layout, explicitly model:

```text
lanes; groups; nodes and roles; relationship types; current vs future;
source of truth; language mode; required edge labels
```

Classify each relationship as call, data flow, dependency, implementation/replacement, optional extension, or synchronization. Implementations and environment alternatives must branch from their common Port (for example `ArchiveStore Port → Local Git Adapter` and `ArchiveStore Port → GitLab Adapter`); never draw an adapter-to-adapter arrow unless it is genuinely a synchronization flow. For layered architectures, check that writes, reads, workers, sources of truth, caches/query projections, and future adapters have explicit roles.
...
Before generating XML, choose Chinese, English, or bilingual labels and size text before placing nodes. Organise by `Lane → Group → Port → Adapter(s)`, keeping a group compact and groups farther apart. Use [layout-rules.md](references/layout-rules.md): sibling gap ≥40px, group gap 50–80px, Port-to-Adapter gap ≥50px, top-level lane gap ≥40px, and 20px content/page margins. Bilingual nodes are ≥200px wide (≥240px for longer descriptions).

Swimlane children use *relative* coordinates. Resolve every child to global coordinates (`parent.globalX + child.x`, `parent.globalY + child.y`) before checking collisions, bounds, or routing. Reserve the lane header: `node.globalY >= lane.globalY + startSize + 12px`. Calculate canvas and lane dimensions from content; when expanding them, also update page dimensions, title centering, right margins, and connection projections.
...
# Straight-first connector (use when nodes are aligned and corridor is clear)
edgeStyle=none;html=1;

# Orthogonal connector (only after a straight route is impossible)
...
### Step 6 — Route, Save, Validate, and Render

1. **Write the file** to the requested path with `.drawio` extension
2. **Route after positioning nodes**. Prefer vertical, then horizontal, then one bend; move nodes before adding hand-authored `mxPoint` waypoints. Use [routing-rules.md](references/routing-rules.md). Check every route against non-endpoint node rectangles with a 10–20px safety margin.
3. **Run structure and layout validators**:
   ```bash
   python scripts/validate-drawio.py <path-to-file.drawio>
   python scripts/validate-layout.py <path-to-file.drawio>
   python scripts/inspect-routing.py <path-to-file.drawio>
   ```
4. **Render and visually inspect** the full page plus lane headers, dense groups, long routes, and bilingual labels:
   ```bash
   scripts/export-preview.sh <path-to-file.drawio> <preview.png>
   ```
   If Draw.io CLI is unavailable, record `render: NOT RUN`; never claim visual QA passed without a preview.
5. **Tell the user** how to open the file:
...
- Treat edge labels as layout objects: reserve 100–160px for bilingual labels and avoid long labels on short connectors
- Use dashed styles and explicit labels for future/optional adapters; distinguish them from current capabilities
...
| XML validates but diagram looks wrong | Geometry was not validated or rendered | Run `validate-layout.py`, export a preview, and inspect it |
| Node is in wrong tier | Relative child coordinates were read as absolute | Resolve global coordinates through all parents and fix parent semantics |
| Zig-zag connectors | Waypoints were added before layout was aligned | Move nodes, use straight projection, then consider one bend |
...
- [ ] For architecture pages, lanes have ≥40px gaps and children do not intrude lane headers
- [ ] Global (not local) coordinates were used for node overlap, bounds, and edge checks
- [ ] Nodes do not overlap, exceed their lane/page, or leave insufficient bilingual text width
- [ ] Edges do not cross non-endpoint nodes; manual waypoint count is zero unless documented
- [ ] Edge labels have sufficient space and do not cover headers, nodes, or other labels
- [ ] Relationship semantics distinguish implementation/replacement from data synchronization
- [ ] A preview was exported and visually inspected, or render was explicitly marked NOT RUN
...
| `scripts/validate-layout.py` | Global-coordinate geometry, lane, header, bounds, edge, and bilingual-width checks |
| `scripts/inspect-routing.py` | Reports hand-authored edge waypoints |
| `scripts/export-preview.sh` | Exports a PNG/SVG through the installed Draw.io CLI, or reports that rendering was not run |
...
| `references/layout-rules.md` | Spacing, lane, global-coordinate, and dynamic-canvas rules |
| `references/routing-rules.md` | Relationship semantics and straight-first routing rules |
| `references/bilingual-labels.md` | Language-mode and bilingual sizing rules |
| `references/visual-qa-checklist.md` | Preview inspection and delivery-report checklist |
