...
Run all three checks before delivery:

```bash
python validate-drawio.py diagram.drawio
python validate-layout.py diagram.drawio
python inspect-routing.py diagram.drawio
./export-preview.sh diagram.drawio diagram.png
```

`validate-layout.py` resolves nested child geometry to global coordinates and fails on overlapping top-level lanes, header intrusion, overlapping nodes, lane overflow, narrow bilingual nodes, and aligned straight edges that cross other nodes. It reports manual waypoint count; architecture pages should normally have zero.
